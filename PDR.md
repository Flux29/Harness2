...
code stays out of `vendor/` (pristine upstream + `patches/` + meta docs only).
Agentic deletion is disabled (ADR-0021): files scheduled for deletion are parked in
a **local-only, git-ignored, never-pushed** `Obsolete/` (git history is the durable
record), while deferred-with-a-future code is quarantined in a **committed**
`eval_optimizer/legacy/` (out of the live import path, Gate-5 enforced).
